...
import com.artesdesign.augmentedReality.databinding.SpecialRvItemBinding
import com.bumptech.glide.Glide
import com.artesdesign.data.Product
     ...
     inner class SpecialProductsViewHolder( val binding: SpecialRvItemBinding):RecyclerView.ViewHolder(binding.root) {
         ...
         fun bind(product:Product){
             binding.apply {
                 ...
                 tvSpecialProductPrice.text = "${product.price} TL"

                 product.offerPercentage?.let {
                     val remainingPricePercentage = 1f - it
                     val priceAfterOffer = remainingPricePercentage * product.price
                     tvSpecialProductPrice.text = "${String.format("%.2f",priceAfterOffer)} TL"
                 }
